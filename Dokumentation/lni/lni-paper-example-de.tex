...
Wichtig bei der Architektur ist zu beachten, dass Client, API und Datenbank allein arbeitende Komponenten sind. 
Jede dieser Systeme ist einzeln aufrufbar und referenziert sich gegenseitig um dem Prinzip eines verteilten Systems zu genügen. 

\subsection{Authentifizierung}

Authentifizierung sorgt dafür, dass ein Konsument gegenüber einem System (Produzent) verifiziert werden kann.
...
Ein, mittels Token gestützes Authentifizierungsverfahren, bietet die Möglichkeit den Konsument mittels 
...
enthalten kann. 
Die zusätzlichen Informationen können von der Client-Anwendung genutzt werden um dem Konsument 
...
\begin{figure}
  ...
  \caption[Authentifizierung mittels Token]{Authentifizierung mittels Token} 
  \label{token-auth}
  \cite{OktaJWTAuth}
\end{figure} 
...
Werden die korrekten Anmeldedaten eingegeben bzw. registriert sich der Konsument an der 
Anwendung, sendet der Server (Produzent) als Antwort einen Token. 
