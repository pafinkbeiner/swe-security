...
Sie reicht dabei vom Schutz einzelner Dateien bis hin zur Absicherung von Rechenzentren und Cloud-Diensten \cite{ITSEC}. 
...
der Vernetzung von mehreren Systemen wird zum späteren Zeitpunkt noch einmal genauer erläutert \cite{NWS}. 
...
Ebenfalls ist eine Initiierung mithilfe von Zertifikaten möglich \cite{IPSec}.
...
Innerhalb eines VPN Netzwerkes ist die Kommunikation geshcützt vor Offenlegung, 
...
nur begrenzt die Möglichkeit liefern die Sicherheit auf der Anwendungsschicht zu unterstüzten.
...
an die Grenze eines ungeschützen Netzwerkes plaziert wird. 
...
Die Verwendung von Firewalls bringt daher den großen Vorteil, das die Kommunikation 
...
In den beiden Quellen der Stanford University und der CERN wird dieses Konzept leicht abgewandelt beschrieben.
...
zunächst seine Identität überprüft. Dies erfolgt über den mit dem Attribut-Zertifikat mitgegebem eindeutigem Name (Distinguished Name). Anschließend kann die Ressource mithilfe des Attribut-Zertifikats und den Richtlinien vom Server den Zugriff erlauben oder ablehnen. CERN schriebt hier, dass
die Entscheidung über Zugriff auf die Ressource direkt vom Access Decision Framework der Ressource erfolgt. Nach Stanford erfolgt dies von PERMIS selber, welcher dann die Entscheidung an die Ressource zurückgibt.
...
Das React-Frontend, die API und die Datenbank sind für sich vollkommen allein stehende Komponenten und sind über die Infrastruktur 
...
liefert einen Eindruk über den jeweiligen Einsatzbereich der Konzepte. 
